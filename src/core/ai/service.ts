...
export const geminiService = {
  async analyzeJobMultimodal(jobText: string, files?: { base64: string, mimeType: string }[]) {
    try {
      ...
      logger.warn('AI analysis failed or timed out, executing local fallback', { reason });
      return normalizeAnalysis(localFallbackAnalysis(jobText || '', reason));
    }
  },
    ...
    for (let i = 0; i < CHAT_FALLBACK_CHAIN.length; i++) {
      ...
      try {
        ...
        logger.logApp('AI Request Success', {
          ...
          feature: 'AI Chat',
          primary_model: CHAT_FALLBACK_CHAIN[0],
        ...
        if (i < CHAT_FALLBACK_CHAIN.length - 1) {
          logger.warn('AI Model Fallback', {
            feature: 'AI Chat',
            failedModel: modelName,
            fallbackModel: CHAT_FALLBACK_CHAIN[i + 1],
            reason: message
          });
